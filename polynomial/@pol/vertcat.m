...
%VERTCAT   Vertical concatenate polynomials
%
% The commmand
%    C = [A;B] 
% returns the vertical concatenation of the polynomial matrices 
% A and B.  A and B must have the same numbers of columns.  
...
%                         $Date: 16-Jun-2000 14:00:00    J. Jezek  $
%                         $Date: 03-Aug-2000 10:00:00    J. Jezek  $
...
if ni>=2,                    % sampling periods checking
   ...
   pom = varargin{i};
      ...
      if isa(pom,'mdf'),             % mdf case
         varargin{i} = pom;
         eval('C = defract(vertcat(varargin{1:ni}));',...
            'error(peel(lasterr));');
         return;
      end;
      if pom.s(2)~=Ps(2),
...
if ni>=2,
   for i=2:ni,
      ...
      Pcoef = cat( 1,cat(3,Pcoef,zeros([Ps,s])) , ...
                     cat(3,pom.c,zeros(pom.s(1),pom.s(2),-s)) );
         
      % new size
      Ps = [Ps(1)+ pom.s(1)  Ps(2)]; 
   end;                  
end;

...
%            C = A + B
%
% C = A + B or C = PLUS(A,B) adds the polynomial matrices A and B 
...
%
% See also POL/MINUS, POL/UPLUS.

%       Author(s):  D. Henrion, S. Pejchova, M. Sebek 24-2-98
%       Copyright (c) 1998 by Polyx, Ltd.
%       $Revision: 2.0 $  $Date: 09-Mar-1998 14:25:34   $
...
if isa(B,'mdf'),
   C = 0;
   eval('C = defract(plus(A,B,tol));','error(peel(lasterr));');
   return;
end;
...
if tv==2,
   C = 0;
   eval('C = plus(tsp(A),tsp(B),tol);','error(peel(lasterr));');
   return;
elseif tv==0,

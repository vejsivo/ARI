...
%RESAMPH1     Resample and first order hold discete time polynomial
%
% For polynomial F in variable 'z','q','z^-1' or 'd',
% the command  G = RESAMPH1(F,K,H,L)  returns polynomial
% or fraction G, the result of first order holding with
...
case {'z^-1','d'},
   ...
   switch Fv,
   case 'z^-1', zizi = zi(Fh);
      F = F*rdiv(zizi*(1-zizi^l)^2,(1-zizi)^2);
   case 'd', dd = d(Fh);
      F = F*rdiv(dd*(1-dd^l)^2,(1-dd)^2);
   end;
   F.h = Fh;
   G = resamp(F,k,h);
...
case {'z','q'},
   F = sdf(F);
   eval('G = resamph1(F,varargin{1:lv});',...
      'error(peel(lasterr));');

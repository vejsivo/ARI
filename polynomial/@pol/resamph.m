...
%RESAMPH     Resample and hold discrete time polynomial
%
% For polynomial F in variable 'z','q','z^-1' or 'd',
% the command  G = RESAMPH(F,K,H,L)  returns polynomial
% or fraction G, the result of holding with interval L
% and resampling with period K and phase H.
%
% For more details, see FRAC/RESAMPH.
...
case {'z^-1','d'},
   ...
   switch Fv,
   case 'z^-1', zizi = zi(Fh);
      F = F*rdiv(1-zizi^l,1-zizi);
   case 'd', dd = d(Fh);
      F = F*rdiv(1-dd^l,1-dd);
   end;
   F.h = Fh;
   G = resamp(F,k,h);
...
case {'z','q'},
   F = sdf(F);
   eval('G = resamph(F,varargin{1:lv});',...
      'error(peel(lasterr));');

function R = mrdivide(P,Q,var)
%MRDIVIDE(/)    Matrix right divide two-sided polynomials
%
% The command  R = MRDIVIDE(P,Q)  or  R = P/Q  performs right
...
% then the result is a right-denominator fraction in variable 'z'
% or 'z^-1', decided standardly by PGLOBAL.DISCRVAR or by an
% optional input argument VAR, e.g.  R = MRDIVIDE(P,Q,'z^-1') .
%
% See also TSP/MLDIVIDE, TSP/RDIVIDE, TSP/LDIVIDE.
...
elseif isa(var,'pol'),
   ...
   if all([vs1,vs2,vd]==1)&(~any(var.c(:,:)-[0,1])),
      ...
      error('Invalid variable symbols.');
   end;
elseif ~isa(var,'char'),
...
if isa(Q,'double') & ndims(Q)==2,
   ...
   N.h = h; D.h = h;
   R = 0;
   eval('R = rdf(N,D);', 'error(peel(lasterr));');
   if strcmp(var,'z^-1'),
      R = reverse(R);
   end;
end;

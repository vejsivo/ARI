...
%POLES   Poles of a scalar-den fraction
%
% The command
%   POLES(F)
% computes the poles of scalar-denominator fraction F.
%
% The option  'all'  requires to compute the infinite poles
% as well. The options METHOD and TOL may specify the method
% of roots computation and the tolerance, as in POL/POOTS.
%
% For fractions in 'z' or in 'z^-1', it is possible to prescribe
% the variable for computing poles, the default being F.v .
% So, e.g.
%    POLES(z*inv(z-0.5))  or  POLES(z*inv(z-0.5),'z') or
%    POLES(inv(1-0.5*zi),'z')
% yields 0.5, whereas
%    POLES(inv(1-0.5*zi))  or  POLES(inv(1-0.5*zi),'zi') or
%    POLES(z*inv(z-0.5),'zi')  
% yields 2.
%
% See also POL/ROOTS.

function D = den(F,R)
%DEN    Denominator of scalar-den fraction
%
% For scalar-den fraction F, the command  D = DEN(F)
% delivers polynomial D, which is denominator of F.
%
% For scalar-den fraction F and polynomial R, the command
...
if nargin==2,
   ...
   eval('F = sdf(F); D = pol(R);','error(peel(lasterr));');
   S.type = '.';
   S.subs = 'den';

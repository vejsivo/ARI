...
%LAURENT      Laurent series of scalar-den fraction
%
% If F is a scalar-den fraction in variables 'z' or 'q' then
...
%
% If F is a scalar-den fraction in variables 'z^-1' or 'd' then
% the command   Q = LAURENT(F,DEGREE)
% returns (possibly two-sided) polynomial Q ... the first
% terms of the Laurent series of F about the point z^-1 = d = 0
% (from positive powers of 'z' down to the power  z^-DEGREE ).
%
% If F is a scalar-den fraction in variables 's', 'p' or '', then
...
% with  R(:,:,I+1) = RI  for  I = 0,1,...DEGREE .
%
...
if isempty(F.frac.v) | strcmp(F.frac.v,'s') | strcmp(F.frac.v,'p'),
   if nargout<=1,
      Q = longldiv(F.frac.num,F.frac.den,Fdegree,Ftol);
   else
      [Q,R] = longldiv(F.frac.num,F.frac.den,Fdegree,Ftol);
      if isa(R,'pol'),
         RR = R{0};
         [sR1,sR2] = size(RR);
   ...
   [QQ,RR] = longldiv(F.frac.num,F.frac.den,Fdegree,Ftol);
   if QQ==0, QQ = pol(RR);
   elseif deg(RR)<=0,
      QQ = QQ+RR;

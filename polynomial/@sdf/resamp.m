...
%RESAMP    Resample discrete time scalar-den fraction
%
% Let F be a scalar-denominator fraction in variable 'z','q','z^-1'
...
% (defaults being  K=2, H=0 ), returns scalar-denominator fraction G
...
%
% For example:
%    F = z*inv(z-0.8) = 1*inv(1-0.8*z^-1)
%    laurent = 1 + 0.8*z^-1 + 0.64*z^-2 + 
%              0.512*z^-3 + 0.0496*z^-4 + 0.32768*z^-5 + ...
%
%    RESAMP(F,3,0) = z*inv(z-0.512) = 1*inv(1-0.512*z^-1)
%    laurent = 1 + 0.512*z^-1 + ...
%
%    RESAMP(F,3,1) = (0.8*z)*inv(z-0.512) = 0.8*inv(1-0.512*z^-1)
%    laurent = 0.8 + 0.4096*z^-1 + ...
%
%    RESAMP(F,3,2) = (0.64*z)*inv(z-0.512) = 0.64*inv(1-0.512*z^-1)
...
%
% See also SDF/LAURENT, POL/RESAMP, TSP/RESAMP.
...
[Q,F.frac.num] = rdiv(F.frac.num,F.frac.den,tol);

Fd = F.frac.den; t = tdeg(Fd);
if isempty(t), t = 0;
end;
Fd = shift(Fd,-t); F.frac.den = Fd;
[sF1,sF2] = size(F); X = pol(zeros(sF1,sF2));
pvart = pvar^t;
for u = 1:sF1,
   for v = 1:sF2,
      [X(u,v),F.frac.num(u,v)] = axbyc(Fd,pvart,F.frac.num(u,v),'miny');
   end;
end;
X = rev(X,t); X.v = avar; X.h = F.frac.h;
   ...
   for x = 1:lkf,
      ...
      if kfx==2,
         ...
         for y = 1:lh,
            ...
            for i = 0:k1,
               ...
               H{y} = plus(H{y}, omegakh^i * sdf(bb,aa), tol);
            end;
            G{y} = H{y}/kfx;
         end;
      end;
      ...
      if strcmp(var,'z^-1') | strcmp(var,'d'),
         for y = 1:lh,
            ...
            G{y} = sdf(resamp(Gyn,kfx,0),resamp(Gyd,kfx,0));
         end;
      else
         for y = 1:lh,
            Gy = G{y}; Gyn = Gy.frac.num; Gyd = Gy.frac.den;
...
elseif strcmp(met,'ss'),
   ...
   GG = sdf(AA,BB,CC,DD,num2str(tol),'z');
   GG.frac.h = Gh;

...
%MTIMES(*)   Matrix multiply scalar-den fractions
%                R = P*Q   or   R = MTIMES(P,Q)
%
% For scalar-den fractions P,Q the command returns their matrix
% product R, which is also a scalar-den fraction.
%
% The number of columns of P must be equal to the number of rows
% of Q unless one of the matrices P,Q is scalar. In such a case,
% this scalar is multiplied with every entry of the second matrix.
%
...
R = 0;
eval('R = sdf(mtimes(P.frac.num,Q.frac.num,tol),mtimes(P.frac.den,Q.frac.den,tol));', ...
   'error(peel(lasterr));');

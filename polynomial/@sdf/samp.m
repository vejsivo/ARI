...
%SAMP   Sample continuous-time scalar-den fraction
%
% Let F be a scalar-denominator fraction in variable 's' or 'p'.
% The command  G = SAMP(F,T,TAU) where scalars T,TAU are
% sampling period, T>=0  and sampling phase,  TAU >=0 ,
% returns proper scalar-denominator fraction G
% in variable 'z','q','z^-1' or 'd', whose Laurent series equals
% to the inverse Laplace transform of F, sampled with period T and
...
%    
%    SAMP(F,T,0) = z*inv(z-exp(-a*T))
%    laurent = 1 + exp(-a*T)*z^-1 + exp(-a*2T)*z^-2 + ...
%
%    SAMP(F,T,TAU) = exp(-a*TAU)*z*inv(z-exp(-a*T))
...
%
% See also SDF/LAURENT, FRAC/LAPLACE, SDF/RESAMP.

%         Author:  J. Jezek  14-Jun-2000
%         Copyright(c) 1999 by Polyx, Ltd.
%         $ Reevision $  $ Date 16-Aug-2000 $
%                        $ Date 16-Oct-2000 $
%                        $ Date 16-Jan-2001 $
%                        $ Date 03-Jan-2002 $
%                        $ Date 30-Sep-2002 $

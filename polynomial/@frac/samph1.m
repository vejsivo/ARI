...
%SAMPH1   Sample and first order hold continuous time fraction
%
% For fraction F in variable 's' or 'p', the command
%  G = SAMPH1(F,T,TAU,L) with T,L >0, TAU >=0, returns proper
% fraction G in the variable 'z','q','z^-1' or 'd', the result
% of first order holding with interval L and sampling with
...
% coincides with 0, L or 2*L, the expression F(s)/s must be
% proper (otherwise, in time domain, sampling of a Dirac
% impulse would be attempted).
%
% For more details and for possible further arguments,
% see RDF/SAMP, LDF/SAMP, MDF/SAMP, SDF/SAMP, FRAC/SAMPH.
...
switch Fv,
case 's', F.den = F.den * s^2;
case 'p', F.den = F.den * p^2;
otherwise,
...
if L==T,
   ...
   for k = 1:ltau,
      ...
      case 'd', dd = d(Gh); 
         Gn = Gn*(1-dd)^2;
      case 'z', zz = z(Gh);
         Gn = Gn*(zz-1)^2; Gd = Gd*zz^2;
      case 'q', qq = q(Gh);
         Gn = Gn*(qq-1)^2; Gd = Gd*qq^2;
      end;
   ...
   taur = [tau,taur2,taur3];
   eval('Gtaur = samp(F,T,taur,varargin{start:lv});', ...
         ['PGLOBAL.DFRACT = sdefr;', ...
         'PGLOBAL.COPRIME = scop;', ...
         'error(peel(lasterr));']);
   Gtaur = reshape(Gtaur,ltau,3);
   Gv = Gtaur{1}.v;
   if isempty(Gv), Gv = PGLOBAL.DISCRVAR;
   end;
   Gh = Gtaur{1}.h;
   ...
   for k = 1:ltau,
      quot2k = quot2(k); quot3k = quot3(k);
      switch Gv,
      case 'z^-1', zizi = zi(Gh);
         Gn = Gtaur{k,1}.num - 2*zizi^quot2k*Gtaur{k,2}.num + ...
            zizi^quot3k*Gtaur{k,3}.num;
         Gd = Gtaur{k,1}.den;
      case 'd', dd = d(Gh);
         Gn = Gtaur{k,1}.num - 2*dd^quot2k*Gtaur{k,2}.num + ...
            dd^quot3k*Gtaur{k,3}.num;
         Gd = Gtaur{k,1}.den;
      case 'z', zz = z(Gh);
         quot = max(quot2k,quot3k); zzquot = zz^quot;
         quot2k = quot-quot2k; quot3k = quot-quot3k;
         Gn = (zzquot*Gtaur{k,1}.num - 2*zz^quot2k*Gtaur{k,2}.num + ...
            zz^quot3k*Gtaur{k,3}.num);
         Gd = zzquot*Gtaur{k,1}.den;
      case 'q', qq = q(Gh);
         quot = max(quot2k,quot3k); qqquot = qq^quot;
         quot2k = quot-quot2k; quot3k = quot-quot3k;
         Gn = (qqquot*Gtaur{k,1}.num - 2*qq^quot2k*Gtaur{k,2}.num + ...
            qq^quot3k*Gtaur{k,3}.num);

...
%PINV   Pseudoinverse of a full-rank fraction matrix.
%
% The command
%     M = PINV(A) 
% returns the pseudoinverse of a fraction A, that is, M such that
% the relations
%   A * M * A = A 
%   M * A * M = M
...
% 
% See also RDF/INV, LDF/INV, MDF/INV, SDF/INV, FRAC/RANK, MDF/RANK.
...
if rankA == min(As),
	...
	if As(1) == As(2),
      eval('M = inv(A,met,tol);',...
         'error(peel(lasterr));');
	elseif As(1) < As(2),
      eval('M = mtimes(A.'',inv(mtimes(A,A.'',tol),met,tol),tol);',...
         'error(peel(lasterr));');
	else
      eval('M = mtimes(inv(mtimes(A.'',A,tol),met,tol),A.'',tol);',...
         'error(peel(lasterr));');
	end;  

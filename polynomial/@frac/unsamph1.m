function F = unsamph1(G,varargin)
%UNSAMPH1    Un- sample and first order hold discrete time fraction
%
% Discrete time fraction G is supposed to be a result
% of sampling and first order holding of continuous time
...
% must not have a higher than single pole for z=0
% (z^-1=inf, q=0, d=inf).
%
...
% are  'ss' - state space
%
% See also FRAC/SAMPH1, FRAC/UNSAMP.
...
if ~all(all(isfinite(absterm))),
   error(['Invalid 1st argument; has pole ', ...
         text,' higher than single.']);
end;

G.num = G.num*zz^2; G.den = G.den*(zz-1)^2;

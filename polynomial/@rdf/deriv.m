...
%DERIV    Derivative of right-den fraction
%
% The command  G = DERIV(F)
% computes the derivative of right-den fraction F. The result is
% also a right-den fraction.
%
...
if n>0,
   for i = 1:n,
      ...
      eval('Nder = deriv(N,var); Dder = deriv(D,var);', ...
         'error(peel(lasterr));');
      eval('[X Y] = axby0(D,-Dder,tol);','error(peel(lasterr));');
      NderY = mtimes(Nder,Y,tol);
      NX = mtimes(N,X,tol);
      num = minus(NderY,NX,tol);
      den = mtimes(D,Y,tol);
      G = rdf(num,den);
   end;
end;   

...
%ISPROPER    Test if right-den fraction is proper
%
% For right-den fraction F, the expression  ISPROPER(F)  returns 1
...
if any(F.frac.s==0),
   ...
   if all(FF.frac.s==1),
      ...
      [D,rk,U] = colred(FF.frac.den,tol);
      N = mtimes(FF.frac.num,U,tol);
   end;

   Nd = deg(N,'col'); Dd = deg(D,'col');
   t = (all(Nd<=Dd));
end;

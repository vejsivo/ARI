function M = rdf(varargin)
%RDF     Create right-denominator fraction
%
% The commmand
%    M = RDF(N,D)
% creates a right-denominator fraction
%    M(v) = N(v) *(D(v))^-1
% from the polynomial matrix N (numerator) and the square nonsingular
% polynomial matrix D (denominator). The sizes of matrices N,D must be
% compatible unless one of them is scalar. Polynomials N,D must be
% finite (Inf, NaN not allowed).
%
...
% The commmand
%    M = RDF(N) 
% returns M = N if N is already a right-den fraction. Otherwise,
% it means  M = RDF(N,1).
%
% Both polynomials N,D should have the same variable symbol. When not,
...
%
% A right-den fraction from polynomials N,D can be also created by
% operation  N/D .
%
% The command
%    M = RDF(A,B,C)  or  M = RDF(A,B,C,D)  or  M = RDF(A,B,C,D,E)
% creates right-den fraction M from state space matrices A,B,C and
...
% RDF(A,B,C,'10^-6'). Also in this case, an optional input 
% argument VAR may specify the variable symbol to be used
% instead the standard one. It must have a form of character string;
% possible values are 's', 'p', 'z', 'zi', 'z^-1', 'q' or 'd'.
%
% The command
%    M = RDF(SYS)
% creates right-den fraction M from Control System Toolbox object
% SYS. Also in this case, the optional arguments TOL or VAR
% may be used.
%
% The command
%    M = RDF(SYMB)
% creates right-den fraction M from Symbolic Toolbox object SYMB.
% Optional argument VAR may be used.
%
% See also POL/MRDIVIDE, POL/RDF, LDF/RDF.

% For the internal structure of object "right-denominator
% fraction", see FRAC/FRAC.
%
%       Author(s):  S. Pejchova, M. Sebek 14-7-99
%       Copyright (c) 1999 by Polyx, Ltd.
%       $Revision: 3.0 $  $Date: 15-Jul-1999 16:20:11   $
%                         $Date: 10-Nov-1999 J. Jezek  $
%                         $Date: 15-Jun-2000 J. Jezek  $
%                         $Date: 31-Jul-2000 D.Henrion $
%                         $Date: 03-Aug-2000 J.Jezek   $
%                         $Date: 01-Nov-2000 J.Jezek   $
...
ni = nargin;
if ni>=1 & isa(varargin{1},'rdf'),      % quick exit
   M = varargin{1};
...
elseif ni>=1 & isa(varargin{1},'sym'),    % symbolic argument
   ...
   % creation of RDF object
   M = 0;
   eval('M = rdf(mdf(polynum,polyden));','error(peel(lasterr));');
   switch var,
   case {'s','p','q','z'}
      M.frac.v = var;
   case {'d'}
   ...
   N = 0; D = 0;
   eval('N = pol(varargin{1});','error(peel(lasterr));');
   if ni==1,
      D = pol(1); PropValStart = 2;
   else
      eval('D = pol(varargin{2});','error(peel(lasterr));');
      PropValStart = 3;
   end;

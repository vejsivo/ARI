...
%SUBSREF    Subscripted reference for right-den fraction
%
...
elseif strcmp(St1,'()'),
   ...
   elseif lS==2,
      Fp = F.frac.p; i = Ssubs{1};
      eval('F.frac.num = F.frac.num(i,:);','error(peel(lasterr));');
      j = Ssubs{2}; n = size(F.frac.den,2);
      if isa(j,'char'),
         if ~strcmp(j,':'),
            error('Invalid 2nd subscript.');
         end;
         e = eye(n);
      else                     %  isa(j,'cell')
         lj = length(j);
         ...
         for k = 1:lj,
            jk=j(k);
            if ~any(jk==(1:n)),
               error('2nd subscript invalid or exceeds matrix dimensions.');
            end;
            e(jk,k) = 1;
         end;
      end;
      P = rdf(F.frac.num,F.frac.den)*e;

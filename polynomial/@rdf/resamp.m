...
%RESAMP    Resample discrete time right-den fraction
%
% Let F be a right-denominator fraction in variable 'z','q','z^-1'
...
% (defaults being  K=2, H=0 ), returns right-denominator fraction G
...
%
% See also RDF/LAURENT, POL/RESAMP, TSP/RESAMP.
...
siz = size(Fd);
[X,F.frac.num] = xaybc(F.frac.den,pvar^t*eye(siz),F.frac.num,'miny');
X = rev(X,t); X.v = avar;
   ...
   for x = 1:lkf,
      ...
      if kfx==2,
         ...
         for y = 1:lh,
            ...
            for i = 0:k1,
               ...
               H{y} = plus(H{y}, omegakh^i * rdf(bb,aa), tol);
            end;
            G{y} = H{y}/kfx;
         end;
      end;
      ...
      if strcmp(var,'z^-1') | strcmp(var,'d'),
         for y = 1:lh,
            ...
            G{y} = rdf(resamp(Gyn,kfx,0),resamp(Gyd,kfx,0));
         end;
      else
         for y = 1:lh,
            Gy = G{y}; Gyn = Gy.frac.num; Gyd = Gy.frac.den;
...
elseif strcmp(met,'ss'),
   ...
   GG = rdf(AA,BB,CC,DD,num2str(tol),'z');
   GG.frac.h = Gh;
   ...
   if lh==1,
      ...
      ii = 1; Gd = GG.frac.den;
      for i = 1:lh,
         G{i} = GG.frac.num(ii:ii+sQ1min1,:)/Gd;
         ii = ii+sQ1;
      end;
   end;

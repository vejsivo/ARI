function [N,D] = ss2lmf(varargin)
%SS2LMF  Convert a state space system to a left polynomial matrix fraction
%
% The commands
%    [N,D] = SS2LMF(a,b,c)
%    [N,D] = SS2LMF(a,b,c,d)
...
% left coprime polynomial fraction representation
%     D^-1 N
% such that
%     D^-1 N = c (xI-a)^-1 b + d(x),
% where x is 's','p' (continuous-time) or 'z','q' (discrete-time)
% or
%     D^-1 N = c x (I - x a)^-1 b + d(x^-1),
...
%  See also the corections in SS2RMF, BHF2RMF

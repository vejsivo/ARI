...
%                  matrix-den fraction
%
% Discrete time matrix-den fraction G is supposed to
...
%  K./(1-z^-1),  K./(1-d) . The result H is equal to G.
%
...
%    'ss' - state space
%
% See also MDF/SAMP, FRAC/UNSAMP, MDF/RESAMP.
...
if strcmp(met,'ss'),
   ...
   if ltaun==1,
      ...
      for i = 1:ltaun,
         ...
         Hid = HH.frac.den(ii:ii+sCC1min1,:);
         H{i} = mdf(Hin,Hid);
         ii = ii+sCC1;
      end;
   end;

...
%ROOTS   Roots of a matrix-den fraction
%
% The command
%   ROOTS(F)
% computes the roots (zero-points) of matrix-denominator
% fraction F.
%
...
% So, e.g.
%    ROOTS((z-0.2)./(z-0.5))  or  ROOTS((z=0.2)./(z-0.5),'z') or
%    POLES((1-0.2*zi)./(1-0.5*zi),'z')
% yields 0.2, whereas
%    ROOTS((1=0.2*zi)./(1-0.5*zi))  or
%    ROOTS((1-0.2*zi)./(1-0.5*zi),'zi') or
%    ROOTS((z-0.2)./(z-0.5),'zi')  
% yields 5.
%
% See also POL/ROOTS.

...
%SUBSREF    Subscripted reference for matrix-den fraction
%
...
%   R(i,j)         the matrix fraction equal to (i,j)-th entry of R
%   R(i1:i2,:)     similarly
...
elseif strcmp(St1,'()'),
   ...
   if lS==1,
      ...
      if isa(X,'pol') | isa(X,'frac'),
         ...
         i = Ssubs{1};
         NN = 0; DD = 0;
         eval('NN = R.frac.num(i); DD = R.frac.den(i);','error(peel(lasterr));');
         P = mdf(NN,DD);
         if strcmp(R.frac.c,'cop'), props(P,'cop',R.frac.tc);
         end;
   ...
   elseif lS==2,
      i = Ssubs{1}; j = Ssubs{2};
      NN = 0; DD = 0;
      eval('NN = R.frac.num(i,j); DD = R.frac.den(i,j);','error(peel(lasterr));');
      P = mdf(NN,DD);
      if strcmp(R.frac.c,'cop'), props(P,'cop',R.frac.tc);
      end;
      if strcmp(R.frac.p,'prop'), props(P,'prop',R.frac.tp);

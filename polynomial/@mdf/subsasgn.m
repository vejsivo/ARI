...
%SUBSASGN    Subscripted assignment for matrix-den fraction
%
% For matrix-denominator fraction R,
%    R.num = Q         sets the numerators of R
%    R.den = Q         sets the denominators
...
if strcmp(St1,'.'),
   ...
   if strcmp(flstr,'den'),
      ...
      eval('R = mdf(R.frac.num,R.frac.den);','error(peel(lasterr));');
      return;
   end;
   ...
   case 'h',
      ...
      elseif isempty(R.frac.v),
         ...
         if Q==0,
            ...
            for i = 1:r,
               for j = 1:c,                
                  [quot(i,j),rem(i,j)] = rdiv(num(i,j),den(i,j));
               end;
            end;
            if deg(rem)>=0 | deg(quot)>0,
               error('Invalid object for zero sampling period.');
            end;
            R.frac.num.h = []; R.frac.den.h = []; R.frac.h = 0;
         else
...
elseif strcmp(St1,'()'),
   ...
   if lS==1 | lS==2,
      eval('Q = mdf(Q);','error(peel(lasterr));');
      eval('R.frac.num = subsasgn(R.frac.num,S,Q.frac.num);','error(peel(lasterr));');
      eval('R.frac.den = subsasgn(R.frac.den,S,Q.frac.den);','error(peel(lasterr));');
      Rd = R.frac.den;
      sd = size(Rd); s1 = sd(1); s2 = sd(2); s12 = s1*s2;
      R.frac.s = sd;
      for i = 1:s12,
         if Rd(i)==0, Rd(i) = 1;
         end;

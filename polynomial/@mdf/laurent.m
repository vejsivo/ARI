...
%LAURENT      Laurent series of matrix-den fraction
%
% If F is a matrix-den fraction in variables 'z' or 'q' then
...
%
% If F is a matrix-den fraction in variables 'z^-1' or 'd' then
% the command   Q = LAURENT(F,DEGREE)
% returns (possibly two-sided) polynomial Q ... the first
% terms of the Laurent series of F about the point z^-1 = d = 0
% (from positive powers of 'z' down to the power  z^-DEGREE ).
%
% If F is a matrix-den fraction in variables 's' or 'p' then
...
row = F.frac.s(1); col = F.frac.s(2); rowcol = row*col;

if isempty(F.frac.v) | strcmp(F.frac.v,'s') | strcmp(F.frac.v,'p'),
   Q = pol(zeros(row,col));
   if nargout<=1,
      for i = 1:rowcol,
         Q(i) = longldiv(F.frac.num(i),F.frac.den(i),Fdegree,Ftol);
      end;
   else
      R = zeros(row,col,Fdegree+1);
      for i = 1:row,
         for j = 1:col,
            [Q(i,j),rem] = ...
               longldiv(F.frac.num(i,j),F.frac.den(i,j),Fdegree,Ftol);
            if isa(rem,'pol'),
               R(i,j,1) = rem{0}; 
            else
               R(i,j,:) = rem;
   ...
   QQ = pol(zeros(row,col)); RR = QQ;
   for i = 1:rowcol,
      [QQ(i),RR(i)] = longldiv(F.frac.num(i),F.frac.den(i),Fdegree,Ftol);
   end;
   if QQ==0, QQ = RR;
   elseif deg(RR)<=0,
      QQ = QQ+RR;

...
%DECLASS     Declass matrix-den fraction (convert to possible lower class)
%
% The command  Q = DECLASS(R)  declasses matrix-den fraction R
...
if ~isempty(R),
   ...
   if strcmp(RR.frac.v,'z') | strcmp(RR.frac.v,'z^-1'),
      [Dd,Dlc] = deg(RR.frac.den,'ent');
      if Dd==tdeg(RR.frac.den,'ent'),
         Q = RR.frac.num./Dlc;
         if any(any(Dd>0)),

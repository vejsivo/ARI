...
%RESHAPE        Change size of left-den fraction
%
% For left-den fraction P, the command Q = RESHAPE(P,M,N)
% returns a M-by-N left-den fraction whose elements are taken
...
eval('P = ldf(P);','error(peel(lasterr));');
P = sdf(P);

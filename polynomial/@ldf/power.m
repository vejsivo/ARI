...
%POWER (.^)  Element-wise power of left-den fraction
%           A.^n
%
% AN = A.^N or AN = POWER(A,N) denotes element-by-element powers.
% The (i,j)-th element of the result is A(i,j)^N(i,j). The left-den
...
eval('An = ldf(power(mdf(A),n,tol));','error(peel(lasterr));');
PGLOBAL.COPRIME = cop;

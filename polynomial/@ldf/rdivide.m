...
%RDIVIDE(./)   Element-wise right divide left-den fractions
%           R = P./Q   or   R = RDIVIDE(P,Q)
%
% For left-den fractions  P,Q  the command returns left-den fraction R
% whose (i,j)-th entry is  P(i,j)/Q(i,j). All Q(i,j) must be nonzero.
%
...
%
% The result is standardly of type 'ldf'. However, it is less time
% consuming to reach the result in type 'mdf'. For that, an optional
% input argument CLASS may be used. It must be a character string,
% possible values 'mdf', 'sdf', 'rdf' or 'ldf'.
%
% See also LDF/LDIVIDE, FRAC/MLDIVIDE, FRAC/MRDIVIDE.
...
eval('R = rdivide(P,Q,tol);','error(peel(lasterr));');
PGLOBAL.COPRIME = cop;

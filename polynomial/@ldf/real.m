function [FR,FI] = real(F,tol)
%REAL   Real part of left-den fraction
%
% For a left-denominator fraction F, the command
% FR = REAL(F)  returns the real part of F. The command
% [FR,FI] = REAL(F) returns also the imaginary part.
%
...
if isreal(F.frac.num) & isreal(F.frac.den),
   ...
   if strcmp(F.frac.p,'prop'), props(FR,'prop');
   end;
end;
...
if nargout==2,
   if isreal(F.frac.num) & isreal(F.frac.den),
      ...
      if strcmp(F.frac.p,'prop'), props(FI,'prop');
      end;
   end;

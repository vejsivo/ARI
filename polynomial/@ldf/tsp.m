function T = tsp(F)
%TSP     Convert left-den fraction to two-sided polynomial
%
% The command  T = TSP(F)  converts left-den fraction F to two-sided
% polynomial. If not possible, error.
%
% The variable symbol of F should be 'z' or 'z^-1'. If not,
% a warning is issued and 'q','s','p' is changed to 'z',
% 'd' is changed to 'z^-1'.
%
% See also FRAC/POL, LDF/DECLASS.
...
if ~isempty(F),
   ...
   Dd = deg(F.frac.den,'col');
   if all(tdeg(F.frac.den,'col')==Dd),
      Dd = repmat(Dd,F.frac.s(1),1);

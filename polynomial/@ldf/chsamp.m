...
%                  left-den fraction
%
% Discerete time left-den fraction G is supposed to
...
% allowed: it must be equal to  (z-1)\K*z  or  (q-1)\K*q,
%  (1-z^-1)\K,  (1-d)\K . The result H is equal to G.
%
...
%    'ss' - state space
%
% See also LDF/SAMP, FRAC/UNSAMP, LDF/RESAMP.
...
if strcmp(met,'ss'),
   ...
   if ltaun==1,
      if taun==tau, BBn = BB;
      else BBn = AA^((taun-tau)/T)*BB;
      end;
   else
      [sBB1,sBB2] = size(BB); sBB2min1 = sBB2-1;
      BBn = zeros(sBB1,0);
      for i = 1:ltaun,
         BBn = [BBn, AA^((taun(i)-tau)/T)*BB];
      end;
   end;
   
   HH = ldf(AAn,BBn,CCn,strtol,'z');
   ...
   if ltaun==1,
      ...
      ii = 1; Hd = HH.frac.den;
      for i = 1:ltaun,
         Hin = HH.frac.num(:,ii:ii+sBB2min1);
         H{i} = ldf(Hd,Hin);
         ii = ii+sBB2;
      end;
   end;

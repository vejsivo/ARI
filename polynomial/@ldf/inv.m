...
%INV    Inversion of left-den fraction
%          G = INV(F)
%
% For a left-den polynomial fraction  F = D^-1 * N ,  the command
% returns  the left-den fraction  G = F^-1  = N^-1 * D .
...
G = 0;
eval('G = ldf(F.frac.num,F.frac.den);','error(peel(lasterr));');

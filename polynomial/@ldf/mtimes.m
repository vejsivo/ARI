...
%MTIMES (*)   Matrix multiply left-den fractions
%                   H = F*G
%
% The command  H = F*G  or  H = MTIMES(F,G)  is the matrix product
% of left-denominator fractions F and G. Any scalar may
...
% fraction is left-den (it corresponds to the first or to the only
...
% be used. An optional 'rdf' or 'ldf' input argument may specify
% the required class of the result.
%
% See also LDF/TIMES.
...
if strcmp(hand,'ldf'),
   if isa(G,'rdf'),
      ...
      Gn = G.num; Gd = G.den; FnGn = 0;
      eval('FnGn = mtimes(Fn,Gn,tol);','error(peel(lasterr));');
      Gs = size(G);
      if Fs(2)~=Gs(1),
         if all(Fs==1),
            H = ldf(rdf(FnGn,mtimes(Fd,Gd,tol)));
         elseif all(Gs==1),
            H = ldf(mtimes(Fd,Gd,tol),FnGn);
         else
            error('Matrices of inconsistent dimensions.');
         end;
      else
         [X Y] = xayb0(Gd,-FnGn,tol);
         Hd = mtimes(Y,Fd,tol);
         H = ldf(Hd,X);
      end;
      ...
      if Fs(2)~=Gs(1),
         ...
         X = 0; Y = 0;
         eval('[X Y] = xayb0(-Gd,Fn,tol);','error(peel(lasterr));');
         Hn = mtimes(X,Gn,tol); Hd = mtimes(Y,Fd,tol);
         H = ldf(Hd,Hn);
      end;
...
elseif strcmp(hand,'rdf'),
   ...
   if Fs(2)~=Gs(1),
      ...
      elseif all(Gs==1),
         H = rdf(ldf(mtimes(Fd,Gd,tol),FnGn));
      else
         error('Matrices of inconsistent dimensions.');
      end;
   else
      [X Y] = axby0(Fd,-FnGn,tol);
      Hd = mtimes(Gd,Y,tol);
      H = rdf(X,Hd);
   end;
   if strcmp(F.frac.p,'prop') & strcmp(G.p,'prop') & ...

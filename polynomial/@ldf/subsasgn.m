...
%SUBSASGN    Subscripted assignment for left-den fraction
%
% For left polynomial fraction F,
...
if strcmp(St1,'.'),
   ...
   if strcmp(flstr,'den'),
      ...
      eval('F = ldf(F.frac.den,F.frac.num);','error(peel(lasterr));');
      return;
   end;
   ...
   case 'v', eval('F.frac.v = R;','error(peel(lasterr));');
   case 'h',
      if isempty(R),
